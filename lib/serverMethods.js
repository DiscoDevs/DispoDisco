...
async function getListByKey({ collectionName, key1, key2, key3, company }) {
...
async function getCompanyName({ username }) {
  const { company, hash } = await collection("users").findOne(
    { username: username },
    { projection: { _id: 0, company: 1, hash: 1 } }
  );
  const bytes = CryptoJS.AES.decrypt(company, hash);
